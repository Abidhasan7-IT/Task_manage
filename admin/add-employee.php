...
<?php  
        ...
        $name = $email = $dob = $gender = $pass = $salary = $Position= "";
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ) {
            ...
            if( !empty($name) && !empty($email) && !empty($pass) && !empty($salary) ){
                ...
                if( mysqli_num_rows($r) > 0 ){
                    ...
                    $sql = "INSERT INTO employee( name , email , password , dob, gender ,Position, salary ) VALUES( '$name' , '$email' , '$pass' , '$dob' , '$gender', '$Position', '$salary' )  ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                     $name = $email = $dob = $gender = $pass =$Position= $salary = "";
                        echo "<script>
